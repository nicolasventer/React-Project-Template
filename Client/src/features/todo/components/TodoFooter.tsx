...
import { DoneFilterValues } from "@/features/todo/types/Todo.type";
import "./TodoFooter.css";

export type TodoFooterProps = { tr: TodoTr; items: Todo[] };
...
export const TodoFooter = ({ tr, items }: TodoFooterProps) => {
	...
	return (
		<footer className="todo-foot">
			...
			<div role="group" aria-label="Filter">
				{DoneFilterValues.map((f) => (
					<button
						...
						onClick={() => todoApp.todos.fn.doneFilter.update(f)}
					>
						{label(f)}
					</button>
				))}
			...
			{doneCount > 0 && (
				<button type="button" className="link clear" onClick={todoApp.todos.fn.todos.completed.clear}>
