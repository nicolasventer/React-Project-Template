...
const IconSun = () => (
	<svg className="darkModeButton-icon" viewBox="0 0 24 24" aria-hidden>
...
export const DarkModeButton = ({ tr }: DarkModeButtonProps) => {
	...
	return (
		<button
			type="button"
			className={clsx("darkModeButton", isDark && "darkModeButton--dark")}
			aria-pressed={isDark}
		...
		>
			<span className="darkModeButton-track" aria-hidden>
				<span className="darkModeButton-thumb">{isDark ? <IconMoon /> : <IconSun />}</span>
			</span>
			<span className="darkModeButton-label" aria-hidden>
