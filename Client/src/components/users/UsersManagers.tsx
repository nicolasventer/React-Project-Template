...
import type { UserOutput } from "@/Shared/SharedModel";
...
export const userColumnManager = new ColumnManager<
	UserFilterKey,
	{
		...
		render: (user: UserOutput) => React.ReactNode;
	}
>({
	email: {
		key: "email",
		...
		render: (user: UserOutput) => user.email,
	},
	role: {
		key: "role",
		...
		render: (user: UserOutput) => user.role,
	},
	lastLoginTime: {
		key: "lastLoginTime",
		headerRender: (props) => (
		...
		render: (user: UserOutput) => formatTime(user.lastLoginTime),
	},
});
