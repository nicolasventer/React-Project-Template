...
import type { ColorSchemeType, LanguageType, LogType } from "../Common/CommonModel";
import { closeHeight, maxConsoleHeight, openHeight } from "./appUtils";
import { _isLanguageLoading, globalState, loadGlobalState, LOCAL_STORAGE_KEY } from "./GlobalState";
...
export const setColorSchemeFn =
	(colorScheme: ColorSchemeType, useTransition: boolean, isColorSchemeLoading: Signal<boolean>) => () => {
		if (useTransition) {
			document.startViewTransition(() => flushSync(() => (globalState.colorScheme.value = colorScheme)));
		} else {
...
export const setLanguageFn = (language: LanguageType, useTransition: boolean) => () => {
	if (useTransition) {
		document.startViewTransition(() => flushSync(() => (globalState.language.value = language)));
	} else {
