...
/** Fields shared by every Elysia handler context (omits route-specific params/status/body). */
export type AppContext = Pick<Context, "request" | "headers" | "path" | "route" | "server">;

export type TokenRequest = Pick<Context, "headers">;
...
// returns true if token is expired
// returns false if token is invalid
const getVerifiedLoginToken = (req: TokenRequest) => {
	const token = req.headers?.["x-token"];
	const loginPayload = !!token && verifyLoginToken(token);
...
const getVerifiedResetPasswordToken = (req: TokenRequest) => {
	const token = req.headers?.["x-token"];
	return !!token && verifyResetPasswordToken(token);
};
